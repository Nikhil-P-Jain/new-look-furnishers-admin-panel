...
export class PurchaseOrderSpecifiedProductComponent implements OnInit {
  ...
  removeAccessory() {
    (this.accessoriesAddEdit.get("accessories") as FormArray).removeAt(
      length - 1
    );
  }
  get f1() {
  ...
  ac_findLength() {
    ...
    this.accessoriesAddEdit.get("accessories").value.forEach((element, i) => {
      if (
        element.accessories_length != null &&
        element.accessories_quantity != null
      ) {
        this.ac_totalLength.push(
          (parseFloat(element.accessories_length) *
            parseFloat(element.accessories_quantity)) /
            1000
        );
  ...
  ac_findArea() {
    ...
    this.accessoriesAddEdit.get("accessories").value.forEach((element, i) => {
      if (
        element.accessories_module != null &&
        this.ac_totalLength.accessories_length != 0
      ) {
        var sum =
          (parseFloat(element.accessories_module) *
            parseFloat(this.ac_totalLength[i])) /
          1000;
        // console.log(sum,"sum");
  ...
  settings1 = {
    ...
    columns: {
      ...
      materialRequisition: {
        ...
        type: "html",
        valuePrepareFunction: (cell, row) => {
          return `<a href=${environment.APP_URL}annexure/material-requisition/${row.annexure_id}>Material Requisition</a>`;
        },
      },
    },
  };
  open2(dialog: TemplateRef<any>, event: any) {
    ...
    this.adservice
      .get_annexure_details_json(this.uniqueId1)
      .subscribe((res) => {
        this.resp4 = res;
        ...
        for (var j = 0; j <= this.products1.length; j++) {
          this.products1.controls[j]
            .get("product_id")
            .patchValue(JSON.stringify(adInfo[j].product_id));
          this.products1.controls[j].get("length").patchValue(adInfo[j].length);
          this.products1.controls[j]
            .get("quantity")
            .patchValue(adInfo[j].quantity);
    ...
    this.acservice.getaccessoriesbyid(this.uniqueId1).subscribe((res) => {
      ...
      for (var j = 0; j < this.accessories1.length; j++) {
        this.accessories1.controls[j]
          .get("accessories_name")
          .patchValue(acinfo[j].accessories_name);
        this.accessories1.controls[j]
          .get("accessories_length")
          .patchValue(acinfo[j].accessories_length);
        this.accessories1.controls[j]
          .get("accessories_quantity")
          .patchValue(acinfo[j].accessories_quantity);
        // this.accessories1.controls[j].get('accessories_total_length').patchValue(acinfo[j].accessories_total_length);
        this.accessories1.controls[j]
          .get("accessories_module")
          .patchValue(acinfo[j].accessories_module);
  ...
  deleteProjectLead(event) {
    ...
    if (window.confirm("Are you sure you want to delete?")) {
      this.adservice
        .delete_annexure_and_details(event.data.annexure_id)
        .subscribe(
          (res) => {
            this.resp = res;
    ...
    this.formAddEdit.get("products").value.forEach((element, i) => {
      if (element.length != null && element.quantity != null) {
        this.totalLength.push(
          (parseFloat(element.length) * parseFloat(element.quantity)) / 1000
        );
        // get('total_length').patchValue()
      } else {
        this.totalLength.push("");
  ...
  findArea() {
    ...
    this.formAddEdit.get("products").value.forEach((element, i) => {
      if (element.module != null && this.totalLength.length != 0) {
        var sum =
          (parseFloat(element.module) * parseFloat(this.totalLength[i])) / 1000;
  ...
  async onSubmit(ref: any) {
    ...
    if (this.flag == false) {
      if (this.formAddEdit.invalid) {
        ...
        this.accessoriesAddEdit.get("accessories").value.forEach((y) => {
          if (y.accessories_name != "") {
            console.log(
              "isnan",
              y.accessories_module,
              typeof y.accessories_module
            );
            ...
            if (y.accessories_module == "") {
              ...
              acinfo.push({
                ...
                accessories_total_length: parseFloat(
                  y.accessories_total_length
                ),
        ...
        if (!this.uniqueId1) {
          ...
          this.adservice.createAnnexure_details(body).subscribe(
            (res) => {
              this.showToast(
                this.success_status,
                this.title,
          ...
          this.adservice.updateAnnexure_details(bo).subscribe(
            (res) => {
              this.resp3 = res;
              this.showToast(
                this.success_status,
      ...
      this.showToast(
        ...
        "Please Input lower amount for area as it exceeds the total quantity mentioned in Purchase Order"
      );
    }
  }
